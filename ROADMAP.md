...
- [ ] **11C.11 — Close the controlled V2→V1 runtime projection decision for the three validated canonical profiles; 23-item metadata-only `keepsakePlan` already completed.**
  ...
  - [ ] **Complete post-integration regression matrix on reviewed `main`:** rerun deterministic canonical generation, full Lua 5.2 UI/scoring/resolver, mechanics/equivalence, Build Registry importer, exact 14-file staging, dependencies/compatibility, install, update/rollback, uninstall and Thunderstore package tests. Record PASS/FAIL/SKIPPED and tested commit. Codex already reported Lua 5.2, canonical/mechanics/staging/diff PASS after `1bafd47` and updater fixture PASS at DEV deployment, but the entire end-to-end matrix has not been demonstrated together afterward; Codex is working on this offline.
  - [ ] **Resolve or explicitly document the PowerShell `ShouldProcess` reliability issue:** the original updater invocation threw `NullReferenceException` at its first `ShouldProcess`, while the PS5.1 invocation with explicit `-Confirm:$false` succeeded. Codex is reproducing and testing only in temporary fixtures; require separate patch review before code changes, preserving confirmations and transactional rollback.
