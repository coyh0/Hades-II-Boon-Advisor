...
Final machine-readable results and logs are in `dist/repair-validation/`. Packaging was exercised only as a test fixture, not a public release. Native files were read for evidence/compatibility. The v0.2 candidate was subsequently installed on DEV through the transactional updater after verifying a fresh persistent backup; its 14-file inventory and preserved settings passed post-update checks. Epic was not accessed. Representative human-controlled gameplay validation is complete; RNG-specific cases that did not occur remain explicitly unobserved.

## Remaining gate

The reviewed repair was merged into `main` through [PR #1](https://github.com/coyh0/Hades-II-Boon-Advisor/pull/1). Transactional deployment to DEV is complete with verified backup and exact inventory. The maintainer controlled Hades directly; screenshots and the complete DEBUG log verified the representative showcase, including Morrigan Hammer behavior and conservative unknown-choice handling. If RNG does not provide a named offer, record it as unobserved. Keep the compact UI, Duos, Hermes and Poms guidance after v0.2. Public updater documentation and the remaining release-readiness gates remain separate.
