...
- [ ] **11C.11 — Generate vetted canonical profiles and introduce informational `keepsakePlan`.**
  ...
  - [ ] **Maintainer authorized targeted DEV-only deployment/smoke test of merged discouraged-label fix (2026-09-25):** deploy reviewed `1bafd4728da5be4d7611ccae8014cbbee938dfcb` to `D:\Dev\Games\HadesII-Dev` only, after a fresh persistent target backup and transactional updater preflight; preserve `config/settings.lua` byte-for-byte (`DEBUG=true`, `UI_TEST_MODE=false`, `BUILD_PROFILE="auto"`), verify 14 installed runtime files (13 staged hashes plus intentionally preserved config), no Epic/live changes. Reuse known safe updater invocation through PowerShell 5.1 with explicit `-Confirm:$false`; keep original invocation reliability defect open for later installer fix. The maintainer's Black Coat Ares Special was **already fully live-tested** on earlier DEV code with screenshot and `LogOutput(6).log` (Ares Special +12, rank 1/2, partial ranking, 0 BoonAdvisor ERROR/WARN), so no repeat Black Coat Ares grind needed to validate this localization-only patch. New UI-label smoke evidence requires a genuine discouraged offer when naturally encountered; existing Lua FR/EN UI tests cover both ranking modes. Deployment is authorized, **not yet confirmed executed**. No further gameplay/scoring modifications, no Epic deploy, packaging, tags or public v0.2 release.
