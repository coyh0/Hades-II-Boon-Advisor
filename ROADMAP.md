...
- [ ] **11C.11 — Generate vetted canonical profiles and introduce informational `keepsakePlan`.**
  ...
  - [ ] **First Black Coat Melinoë Poseidon DEV screenshot captured; awaiting matching DEBUG log (2026-09-25):** screenshot shows a fully displayed three-item order: Rare Poseidon Attack `Frappe marine` rank 1 with `Core · Build`; Poseidon Cast `Glyphe diluvien` rank 2 with `Core` label; Poseidon Sprint `Ruée torrentielle` tied rank 2 with `Utilitaire · Build`. The current canonical `black_coat_melinoe_intermediate` on `main` has `PoseidonWeaponBoon` Attack core, `PoseidonSprintBoon` Sprint core, **Cast open**, and the sole `ForcePoseidonBoonKeepsake` autoSignal. Cast `Core` could reflect generic empty primary slot fill rather than canonical Cast priority. Do not mark auto-profile selection, individual scores, complete ranking or Origination behavior confirmed until the same-session log supplies `BuildId`, `ProfileMode`, resolver/autoSignal evidence, `RankingMode`, all per-offer IDs/reasons/scores and any WARN/ERROR. Preserve screenshot, no scoring/UI modification on visual evidence alone.
