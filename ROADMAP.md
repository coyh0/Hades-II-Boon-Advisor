...
- [ ] **11C.11 — Generate vetted canonical profiles and introduce informational `keepsakePlan`.**
  ...
  - [ ] **Post-deployment in-game discouraged-label smoke check (DEV-only):** launch only HadesII-Dev and capture a naturally encountered Morrigan `ApolloSpecialBoon` offer (or any valid `BUILD_DISCOURAGED` offer) showing `Core · Déconseillé` with corresponding fresh DEBUG `BuildId`, `RankingMode`, reasons, clean hook and no BoonAdvisor ERROR/WARN; if RNG does not provide one, do not claim it occurred—the new full-ranking FR and partial-ranking EN automated tests are Codex-reported PASS. No extra Black Coat Ares grind: that exact Special was already live validated on prior runtime. Keep save/Cloud backup and Epic untouched verification open until independently established.
