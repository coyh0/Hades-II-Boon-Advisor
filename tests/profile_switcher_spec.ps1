...
[IO.File]::WriteAllText((Join-Path $target 'data\builds\registry.lua'), 'return {' + $nl + '    coat_melinoe_intermediate = { selectionKey = "coat_melinoe_intermediate" },' + $nl + '    intermediate = { selectionKey = "intermediate" },' + $nl + '    morrigan_meta = { selectionKey = "morrigan_meta" },' + $nl + '}' + $nl)
& $script -Profile intermediate -TargetPath $target | Out-Null
$changed = [IO.File]::ReadAllText($settings)
if ($changed -ne $original) { throw 'Idempotent Intermediate set rewrote settings.' }
& $script -Profile coat_melinoe_intermediate -TargetPath $target | Out-Null
$changed = [IO.File]::ReadAllText($settings)
if (($changed -notmatch 'BUILD_PROFILE\s*=\s*"coat_melinoe_intermediate"') -or
    ($changed -notmatch 'DEBUG\s*=\s*true') -or ($changed -notmatch 'UI_TEST_MODE\s*=\s*true') -or
    ($changed -notmatch 'OTHER\s*=\s*"preserve"')) {
    throw 'Intermediate -> Black Coat changed unrelated settings.'
}
& $script -Profile intermediate -TargetPath $target | Out-Null
if ([IO.File]::ReadAllText($settings) -notmatch 'BUILD_PROFILE\s*=\s*"intermediate"') {
    throw 'Black Coat -> Intermediate failed.'
}
...
if ($shown -notmatch 'coat_melinoe_intermediate' -or $shown -match '(?m)^Available profiles:.*\bstarter\b') { throw '-Show inventory is stale.' }
...
Assert-Fails 'missing settings' { & $script -Profile intermediate -TargetPath (Join-Path $root 'missing') }
Assert-Fails 'retired Starter profile' { & $script -Profile starter -TargetPath $target }
