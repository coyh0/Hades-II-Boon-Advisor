...
**11C.11 — Review and DEV-validate the locally implemented five-recommendation v0.2 showcase across Morrigan Meta and Black Coat Melinoë; offline gates pass, new live evidence is pending**

Published release: `v0.1.2` (Sister Blades only). Future-`v0.2` `main` has three active runtime profiles: Sister Blades Melinoë Intermediate, Sister Blades Morrigan Meta and Black Coat Melinoë Intermediate; Starter was removed from future-v0.2 in `3201282`. Runtime inventory: **14 files**. All three profiles now have reviewed representative live DEV screenshots and matching DEBUG logs; the subsequent `BUILD_DISCOURAGED` localization fix was merged as `1bafd47`, deployed DEV-only with a verified pre-update backup per Codex, and its French `Core · Déconseillé` label passed a new in-game screenshot/log smoke test after two rerolls. Codex reports 13 staged-file hash matches plus byte-preserved DEV settings (`DEBUG=true`, `UI_TEST_MODE=false`, `BUILD_PROFILE="auto"`), with Epic not accessed; full primary-save backup integrity and independent Epic filesystem checks are not established here. No new v0.2 UI, speculative scoring, Epic deployment or public release was authorized.
...
Community Curator completed the private Sheet migration and the maintainer approved all **378** documentary recommendations across 12 guides (118 in the three approved v0.2 profiles; 260 deferred); 11C.9/11C.10 are complete. All Registry v2 rows remain `documentation_only`; documentary approval does not independently validate every native ID/mechanic or make a row runtime-importable. The 118-row inventory is complete. All **23** four-phase `keepsakePlan` metadata records (8 Melinoë / 9 Morrigan / 6 Black Coat) remain informational, non-executable and have no v0.2 UI. On 2026-09-25 the maintainer approved a five-row v0.2 showcase: Morrigan `DaggerTripleBuffTrait` Hammer priority 1 and `DaggerAttackFinisherTrait` alternative 2; Black Coat `HestiaWeaponBoon` / `ZeusWeaponBoon` Attack alternatives and `SuitAttackSizeTrait` Hammer alternative 3. The repaired branch and its 15-suite offline gate are pushed, and draft PR [#1](https://github.com/coyh0/Hades-II-Boon-Advisor/pull/1) is open for shared review. The candidate is transactionally deployed to DEV with a verified backup; fresh gameplay validation remains open. No release has occurred. Do not promote the remaining 113 active rows or 260 deferred rows. No compact build-status tile or other build-guidance UI in v0.2. The 34 unknown historical source dates are optional and non-blocking. Historical 15-file and Starter statements below describe earlier milestones, not the current runtime. Hammer rerolls are not a game mechanic.
...
- **Maintainer-approved v0.2 profile scope (currently present on main; release still gated).**
  - Sister Blades — Melinoë Intermediate: active runtime profile; unconditional base Ares Attack corrected in 11C.8 and independently live DEV-validated (+13/full). Zeus Special and a new representative Melinoë Hammer were not observed in the later three-profile logs.
  - Sister Blades — Morrigan Meta / Blood Triad: active runtime profile with live Hera Attack, Hephaestus Special, Apollo partial/full ranking, unranked Hammers and updated French `Déconseillé` label validated. Ares Special and unimplemented Blood Triad-specific Hammer rules remain untested/deferred.
  - Black Coat — Melinoë Intermediate: part of the maintainer-approved 118-row v0.2 documentary scope; Poseidon start, planned speed Hammer and Ares Special independently live DEV-validated. Conditional Launcher Frame and keepsake autoSignal causality remain unproved; release-level gates remain open.
...
- [ ] **11C.11 — Define and deliver an explicitly approved, evidence-gated v0.2 capability showcase for the three validated canonical profiles; 23-item metadata-only `keepsakePlan` completed.**
  ...
  - [x] **Decide the v0.2 showcase source→runtime delta:** the maintainer approved the five-row selection documented in `data/projections/v02-showcase/rows.json`. Preserve Melinoë's unconditional base `AresWeaponBoon` fix. Keep Morrigan Specials, Blood Triad predicates, conditional Launcher Frame and all unselected rows unchanged/documentary-only. No other Registry row was promoted.
  ...
  - [x] **Earlier targeted native-evidence triage (2026-09-25; superseded by the selected showcase review below):** prior read-only report proposed keeping existing Specials and deferring new Hammers/alternatives pending owner decision. The maintainer has since approved five specific additions after a fresh DEV 139606 native-source review. This earlier triage did not establish live behavior; fresh gameplay evidence for the new additions is still pending.
  - [x] **Complete the 118-row decision matrix and verify the five selected deltas (2026-09-25):** the complete read-only matrix and source snapshot are in `docs/audit/v02-registry-matrix.md` and `docs/audit/v02-registry-source.json`. It distinguishes 23 existing Keepsake metadata entries, 31 intersections with existing canonical/runtime behavior, five selected deltas, and 59 deferred recommendations/guidance records. Native DEV 139606 text and mechanics were re-read for all five IDs; exact-name candidates that remain unresolved are recorded as such. Full evidence and row decisions: `docs/audit/V02_AUDIT_REPORT.md`. The 260 future records remain deferred.
  - [x] **Maintainer v0.2 showcase recommendations and UI scope approved (2026-09-25):** focus v0.2 on richer verified recommendations/mechanics across the three active profiles. The maintainer subsequently approved the five selected Registry recommendations above. Keep the compact build-status tile, phased Keepsake HUD, Legendary/Duo panel and other build-guidance UI after v0.2; `keepsakePlan` remains metadata-only. Approval of the five rows does not establish live validation.
  - [x] **Earlier consolidated proposal (2026-09-24; planning only):** inventoried A/B/C and proposed metadata-only `keepsakePlan`. Superseded for the selected five showcase rows by the maintainer-approved proposal below. The V1 importer still does not ingest `keepsakePlan`.
  - [x] **Approve and prepare the curated nonzero five-row Registry v2→V1 projection (2026-09-25):** the maintainer approved two Morrigan Hammers and three Black Coat recommendations after review of their IDs, native mechanics, expected scorer effects and fallback behavior. Projection rows/policy retain Registry recommendationIds, source fingerprints, documentary conditions/priorities and independent local ID attestation in `data/projections/v02-showcase/`. The separate 23-item `keepsakePlan` remains metadata-only. This is a selected delta, not whole-profile import and not a change to Registry source verification statuses.
  - [x] **Prepare and review the v0.2 showcase proposal before implementation (2026-09-25):** the 118-row audit and five-row shortlist are documented in `docs/audit/V02_AUDIT_REPORT.md`. It distinguishes existing behavior, approved recommendations, unresolved conditions and deferred rows. The selected conditions are treated as profile intent and are never interpreted as runtime predicates. Full/partial/none, replacement, Ares and Hammer fallback scenarios are specified and tested offline. No resolver or UI behavior is added.
  - [x] **Implement the five approved profile deltas and pass the local offline gate (2026-09-25):** the repaired branch preserves the five exact canonical/generated recommendations; restores the formerly corrupted roadmap, full scoring suite and canonical PowerShell test; and adds the five-row projection/policy plus fail-closed and scoring regressions. Canonical profiles, 23 Keepsake records, autoSignals, mechanics templates, scorer, resolver and UI were compared with clean `main`; only the five approved profile records changed. **15 suites PASS**: canonical/deterministic generation and full Lua 5.2; canonical mechanics/equivalence; Registry and Black Coat importers; v0.2 projection; profile switcher; 14-file staging; compatibility; install/update/rollback/uninstall; Thunderstore/package fixtures; and `git diff --check`. Windows PowerShell 5.1 child module paths were explicitly isolated after an initial test orchestration failure. Detailed evidence: `docs/V02_SHOWCASE_REPAIR.md`; machine results: `dist/repair-validation/results.json`. This closes local implementation/offline validation only; fresh live DEV validation and release remain open.
  - [x] **Complete local review and create the v0.2 candidate commit (2026-09-25):** consolidated diff against clean main is `dist/repair-validation/v02-showcase-consolidated.patch`; the repair was first recorded in local commit `47f496b48b89e01c6519ad3d509215305c3cf031` on `codex/v02-mechanics-showcase`. Fifteen offline suites pass. The first push attempt was blocked until the maintainer confirmed the remote repository; the branch was subsequently pushed and a draft PR opened (see below). It is not merged to `main`. Preserve the three SHA-verified historical review patches.
  - [x] **Push the reviewed candidate branch for shared integration (2026-09-25):** pushed `codex/v02-mechanics-showcase` to the maintainer-confirmed repository and verified the remote tracking ref. Opened draft PR [#1](https://github.com/coyh0/Hades-II-Boon-Advisor/pull/1) against `main` for review. It is not merged.
  - [x] **Deploy the repaired v0.2 candidate to DEV transactionally (2026-09-25):** game closed; `Hades2.exe` version 139606; staging and runtime compatibility PASS. Created and recursively SHA-256 verified persistent backup `D:\Dev\Games\HadesII-Dev\Ship\ReturnOfModding\backup\Local-HadesIIBoonAdvisor-pre-v02-showcase-20260925-204854` (14 files/3 directories). The official `tools\Update-BoonAdvisor.ps1` ran via Windows PowerShell with explicit `-Confirm:$false`; a normal-permission attempt was denied before mutation, then the exact installed target was reconfirmed against the backup before the elevated retry. Post-update DEV inventory is exactly 14 files/3 directories; all 13 non-settings files match staging, `config\settings.lua` retains SHA-256 `087285B0176E195FAA1182396BC8D56390A0B9C1810C07E6E0D27BC97978F10F` and settings `DEBUG=true`, `UI_TEST_MODE=false`, `BUILD_PROFILE="auto"`; no transaction remnants. The DEV manifest remains `0.1.2` by design pending the release gate. Only `HadesII-Dev` was accessed; Epic was left untouched. No gameplay has yet been observed.
  - [ ] **Validate the five new recommendations in fresh DEV gameplay under human control:** the maintainer prefers to control Hades directly; do not automate game input. On natural offers, capture screenshots and the complete DEBUG log at `D:\Dev\Games\HadesII-Dev\Ship\ReturnOfModding\LogOutput.log`. Cover Morrigan Hammer recommendations (`DaggerTripleBuffTrait`, `DaggerAttackFinisherTrait`) and Black Coat Attack alternatives (`HestiaWeaponBoon`, `ZeusWeaponBoon`) plus Hammer (`SuitAttackSizeTrait`); retain correct profile/IDs, scores, coverage and full/partial/none fallbacks. If RNG does not provide an offer, record it as unobserved; do not infer Launcher Frame conditions or Blood Triad frequency. The installed build is ready; wait for maintainer-provided captures/logs to assess gameplay. The existing `config/settings.lua` uses the Windows `Profile1` with cloud saves; the maintainer previously accepted this DEV testing context.
  ...
  - [x] **Implement and test metadata-only `keepsakePlan` for the three existing profiles (committed and pushed):** all 23 recommendations (8 Melinoë / 9 Morrigan / 6 Black Coat) added to canonical JSON and corresponding generated Lua with exact approved documentary priorities; optional four-phase schema, non-executable `conditionText`, stable recommendationId, structural negatives and deterministic generation added. GitHub commit `05af0e3391daf12398758cea522b85610d0a6eb5` verified on `main` as a single fast-forward from `3325292`, exactly 8 files and 381 additions / 0 deletions; Black Coat's `ForcePoseidonBoonKeepsake` is the sole existing autoSignal. Codex reports fresh local PowerShell 5.1 canonical, full Lua 5.2, 14-file staging and `git diff --check` PASS; these tests were not independently rerun. No new UI, scoring/resolver, gameplay, DEV/Epic deployment, package or release. This completes the metadata-only subtask, **not** the broader 11C.11 runtime-item projection and DEV gates.
  - [x] **Local metadata-only implementation submitted, independently patch-reviewed, and subsequently integrated:** eight tracked files modified only (three canonical JSON profiles, three corresponding generated Lua modules, generator `tools/Generate-BoonAdvisorProfiles.ps1`, and `tests/canonical_profiles_spec.ps1`); optional four-phase `keepsakePlan` metadata, non-executable text and retained `recommendationId`; no change claimed to runtime scoring, resolver, slots, Hammers or existing Black Coat Poseidon autoSignal. Codex reports canonical PowerShell 5.1, deterministic generation, JSON/Lua parity, Lua 5.2, 14-file staging and `git diff --check` PASS. These are Codex-reported tests, not independently rerun; the complete revised eight-file diff has now been statically reviewed; integrate only after safe Git synchronization and a fresh full local test run, with DEV/Epic deployment still unauthorized. Codex local HEAD was `70691e2`, behind current remote ROADMAP-only commits; preserve untracked `ares-11C8-review.patch` before any later safe sync.
  - [x] **Initial independent patch review identified required corrections, now resolved in the revised patch:** the uploaded UTF-8/no-BOM patch SHA-256 `986077C6E74D7D1EEDAC6FA2EEF7D92009CF460C38703D6C716E8BD2C8CE8680` has exactly the expected eight paths (321 additions, no deletions) and matching GitHub preimage blob SHA prefixes. All 23 new Keepsake entries match their generated Lua representations; no old runtime-driving JSON fields were changed. Direct comparison with the current 378-row approved Registry v2 found **four incorrect `documentaryPriority` copies**: Melinoë R3 Experimental Hammer 1→2, Morrigan R2 Harmonic Photon 1→2, Morrigan R3 Knuckle Bones 1→2, Morrigan R3 Luckier Tooth 1→2 (values shown are patch→Sheet). The new missing-phase test fixture introduces malformed JSON, so it can pass without exercising the intended phase validator. Generator checks nonempty and duplicate phase-local trait IDs, but does not attest native ID trust or Registry recommendationId membership. The subsequent full eight-file patch fixes the four Sheet priority discrepancies, rewrites the missing-phase negative fixture as valid JSON with `Final` removed, and extends tests to all three profiles with negative cases. The helpers check validator failure exit status without asserting the exact failure message; maintain this limitation in review notes.
  - [x] **Revised `keepsakePlan` patch independently accepted for metadata-only Git integration (2026-09-24):** uploaded UTF-8/no-BOM patch SHA-256 `EABD625780B358FA5F29FEF7B848805C03CAAB6A0767F66A91AD7B5CB96C013F`, exactly 8 paths/381 additions/0 removals, all 8 preimage blob SHA prefixes match current GitHub `main`. Static parsing confirms JSON↔generated-Lua parity of all 23 records (8 Melinoë, 9 Morrigan, 6 Black Coat), metadata-only insertions, and direct match of all 23 `recommendationId`/profile/phase/classification/priority/condition fields to the current documentary-approved Sheet. The missing-phase fixture now serializes valid JSON without `Final`; added tests cover all 23 entries and specified structural negatives. Generator validates structure, **not** source membership or native mechanical eligibility. Codex reports canonical PowerShell 5.1, Lua 5.2, deterministic generation, 14-file staging and `git diff --check` PASS; these tests were **not independently rerun** here. The next-gate integration steps were subsequently reported complete; GitHub independently confirms the expected 8-file commit on `main`, while local test PASS and patch preservation are Codex-reported. No DEV or Epic deploy and no v0.2 release authorized.
  - Import only separately approved, mechanically verified runtime rows; the four-phase `keepsakePlan` remains metadata-only, **without in-game keepsake guidance UI for v0.2**. Do not change RNG, offers, saves, scoring or player choice.
  - Add/expand resolver regression generation where practical as part of this work rather than as a separate open-ended task.
  - [x] **Existing offline regression suite re-run reported by Codex (2026-09-25, local `fd64d11`, no code/test modifications):** full Lua 5.2 suite (including prior Melinoë base Ares without Wounds, five planned Sister Blades Hammers, no Dancing Knives promotion, Morrigan/Black Coat/partial ranking/UI/lifecycle, Attack replacement and Origination), canonical profiles, canonical mechanics and equivalence (explicit Python/Lua DLL paths), staging twice with exactly 14 files and matching hashes, Black Coat importer, Build Registry importer, and `git diff --check` all reported PASS. The equivalence test initially failed when required executable paths were omitted; it passed with explicit paths. This is a Codex-reported test run, not independently executed here. No new tests were necessary according to Codex's report and no tracked files were changed. **Offline success does not establish live DEV behavior, verify unimplemented Blood Triad/Launcher predicates, or close the outstanding V2→V1 runtime projection.**
  - [x] **Maintainer explicitly authorized DEV-only three-profile deployment/test (2026-09-25):** transactional deployment **exclusively** to `HadesII-Dev` after verified backup, preserve DEV `config/settings.lua` and `DEBUG=true`, and validate Sister Blades Melinoë Intermediate, Sister Blades Morrigan Meta and Black Coat Melinoë Intermediate using fresh in-game screenshots/logs. This authorizes DEV deployment and tests only, not Epic/live installation, package, v0.2 publication, new runtime scoring/autoSignals, or speculative mechanics. DEV deployment subsequently reported complete on 2026-09-25 with 13 matching staged hashes and byte-preserved settings; **fresh live gameplay/profile evidence remains pending**.
  - [x] **DEV-only preflight and backup verified; deployment safely blocked before mutation (Codex report, 2026-09-25):** local `ade1ded`, both untracked review patches preserved, DEV `Hades2` not running, 14-file timestamped DEV backup `D:\Dev\Games\HadesII-Dev\Ship\ReturnOfModding\backup\Local-HadesIIBoonAdvisor-pre-v02-20260925-004826` recursively matched previous target (`DIFFCOUNT=0`). Rebuilt 14-file staging, DEV `139606` compatibility/native anchors and loader hash reported PASS. Two attempted transactional updates failed at the **first** `$PSCmdlet.ShouldProcess(...)` in `tools/Update-BoonAdvisor.ps1` with NullReferenceException before any target mutation; target still equals backup (`TARGET_BACKUP_DIFF=0`), `config/settings.lua` retains `DEBUG=true`, `UI_TEST_MODE=false`, `BUILD_PROFILE="auto"`; Epic untouched. Three generated profile modules still differ from staging, so **DEV validation has not started**. A prior ROADMAP milestone documented a similar child-process ShouldProcess failure that succeeded on direct invocation; diagnose execution host/invocation context using an isolated update fixture before retrying real DEV. Do not bypass the transactional updater or count this as a successful deployment.
  - [x] **DEV-only transactional updater unblocked and deployment verified by Codex report (2026-09-25; no gameplay test yet):** PowerShell 5.1.26100.9444 ConsoleHost reproduced the first-`$PSCmdlet.ShouldProcess` null reference in an isolated fixture for the original invocation. The fixture succeeded with explicit noninteractive confirmation; the official updater was then invoked via `powershell.exe -NoProfile -ExecutionPolicy Bypass -Command` with escaped `-Confirm:$false`, **not** via manual file copy. `tests/update_spec.ps1`, full Lua 5.2 and staging/DEV 139606 compatibility checks reported PASS; all 14 DEV files present with 13 staging→target SHA-256 matches and the one intentional configuration exception. Existing pre-v0.2 backup `D:\Dev\Games\HadesII-Dev\Ship\ReturnOfModding\backup\Local-HadesIIBoonAdvisor-pre-v02-20260925-004826` retained and checked. Preserved DEV settings byte-for-byte: `DEBUG=true`, `UI_TEST_MODE=false`, `BUILD_PROFILE="auto"`. No leftover transactional directories; Epic unaccessed. The updater's first-ShouldProcess failure remains a reliability issue in the original invocation form, so document/reproduce it as part of the later installation release gate. Manifest reports `0.1.2 → 0.1.2` intentionally; this DEV install carries future-v0.2 code without release version bump. No independent access to the user's DEV filesystem here.
  - [x] **Fresh Melinoë Intermediate Ares DEV offer verified against screenshot and `LogOutput(2).log` (2026-09-25, single observed offer at `0:59:54.419`):** correct `BuildId=sister_blades_melinoe_intermediate`, `WeaponDagger`/`DaggerBackstabAspect`, `Source=AresUpgrade`, Ares Rare Attack on previously empty slot (`Alignment=ALTERNATIVE`, `CurrentSlotTrait=nil`, `SlotStateBefore=EMPTY`, `AspectInteraction=ASPECT_COMPATIBLE`), `RankingReady=true`, `RankingMode=full`, `Score=13` (8 empty primary +4 aspect +1 rarity), `Covered=true`, `Complete=true`, Rank 1; Ares Cast 8 Rank 2, Sprint 4 Rank 3. Screenshot shows the corresponding French order and UI labels. No `ATTACK_BRANCH_UNRESOLVED`, BoonAdvisor ERROR or WARN in supplied session log. The log has two separate hook/ready announcements about 12s apart during script reinitialization, each reporting `hook count=1`, and **one** actual detected boon offer; no duplicate per-offer event observed, but global single-hook lifetime not proven. Origination active, zero owned status families, so combinations unverified. **Save-environment disclosure accepted by maintainer (2026-09-25):** log loads Windows user's `Saved Games\\Hades II` `Profile1` and `PlayerOptInToCloudSaves=true`, so DEV test saves are not isolated; maintainer explicitly confirmed this is intentional and chose to continue without switching Windows accounts. Do not misrepresent main-save backup as verified; recommend a full copy if not already made. Continue authorized DEV-only testing with acknowledged save/cloud risk. Screenshot/log support the Ares offer only, not Zeus Special, Hammers, Morrigan or Black Coat. No release authorized.
  - [x] **Fresh Morrigan Meta Hera DEV offer verified against screenshot and full `LogOutput(3).log` (2026-09-25, 1:07:15):** exact `BuildId=sister_blades_morrigan_meta`, `ProfileMode=meta`, `WeaponDagger`/`DaggerTripleAspect`, starting `ForceHeraBoonKeepsake`, empty core slots, `RankingReady=true`, `RankingMode=full`, three offers `Eligible=true`, `Covered=true`, `Complete=true`. Hera Rare Attack `HeraWeaponBoon` scored **13** (empty-primary +8, build core +4, rarity +1), rank 1; Hera Sprint `HeraSprintBoon` scored **0**, rank 2; Hera Cast `HeraCastBoon` scored **−4** (`BUILD_SLOT_POLICY_DELTA` −4) with `Alignment=NON_TARGET`, `SlotPolicy=reserved`, `SlotConflict=true`, rank 3; UI screenshot correctly shows original card order with displayed ranks 1/3/2, `Core|Build` and `Conflit` labels. The attached log contains **two** actual detected boon offers total (prior Melinoë Ares + this Morrigan Hera), each one detection. It also contains **four** hook-installed/ready announcements across script reinitializations, all `hook count=1`; do not claim global single-hook lifetime. No `[BoonAdvisor] ERROR` or `WARN` match in supplied log. Origination active with `OwnedStatusFamilies=none`; no combination behavior proven. Existing use of Windows primary `Profile1` is explicitly acknowledged by maintainer, not an isolated DEV test save. This validates only the observed Morrigan Hera offer, **not** the pending Morrigan Ares/Hephaestus Specials, Hammers/Blood Triad or Black Coat.
  - [x] **Morrigan Apollo live partial-ranking confirmed using matching screenshot and full `LogOutput (2).zip` (`LogOutput.log`, 2026-09-25, 1:12:30):** `BuildId=sister_blades_morrigan_meta`, `RankingMode=partial` (two covered/complete choices). Heroic Apollo Attack `ApolloWeaponBoon` replaces Epic Hera core Attack (`TraitToReplace=HeraWeaponBoon`), scores **−3** (core replacement −4, rarity difference +1), displayed rank 2/2. Common Apollo Special `ApolloSpecialBoon` scores **+6** (fill empty Special +8, **BUILD_DISCOURAGED −2**), displayed rank 1/2; it remains `Alignment=NON_TARGET`/discouraged in canonical, not `core` or `preferred`. `DoubleStrikeChanceBoon` has `Covered=false`, `Complete=false`, `ORIGINATION_UNRESOLVED`, correctly unranked. The earlier `Core · Build` UI annotation on Apollo Special is **misleading, not a wrong profile or arithmetic fault**: `FILL_EMPTY_PRIMARY_CORE` maps to `Core` and `BUILD_DISCOURAGED` maps to the same unqualified `Build` label as positive build bonuses. Fix the negative UI reason label separately with FR/EN regression tests before public v0.2 release; preserve scoring/coverage and the no-new-UI-feature decision.
  - [x] **Morrigan first live hammer offer correctly unranked (`LogOutput (2).zip`, 1:10:48):** `Source=WeaponUpgrade`, `BuildId=sister_blades_morrigan_meta`, three offered IDs `DaggerTripleRepeatWomboTrait` (Spectral Echo), `DaggerSpecialLineTrait` (Precision Knives), `DaggerTripleHomingSpecialTrait` (Banshee-like special Hammer) each score 0, `Covered=false`, `Complete=false`; `RankingReady=false`, `RankingMode=none`, `INCOMPLETE_ANALYSIS` with 3 unevaluated choices. User screenshot agrees; native identification does **not** authorize unreviewed `hammerPlan` import. Following acquisition, `DaggerTripleRepeatWomboTrait` is recorded as owned in later offers.
  - [x] **Morrigan Hephaestus live Special validated against screenshot and same-session ZIP log (1:22:45):** `BuildId=sister_blades_morrigan_meta`; `HephaestusSpecialBoon` mapped `PREFERRED`, empty Special, scores **+10** (+8 fill +2 preferred), `Covered=true`, `Complete=true`. `HephaestusSprintBoon` (`Covered=true`, `Complete=false`) and Epic `HeavyArmorBoon` (`Covered=false`, `Complete=false`) both have unresolved Origination status knowledge; thus `RankingReady=false`, `RankingMode=none`, `INCOMPLETE_ANALYSIS` with 2 unevaluated choices. `Core · Build` accurately describes slot-fill and positive preference **here**, but does not mean Hephaestus is canonical `core`; it is preferred. No claim that it outranks the other two. ZIP includes 8 actual offer detections, 5 hook-init announcements each `hook count=1`, one known `TryUpgradeBoon` redraw (Hera Rare→Epic), 0 BoonAdvisor ERROR/WARN. This validates observed Morrigan Hephaestus Special **coverage/scoring only**, not all Specials/Blood Triad or full ranking.
  - [x] **Misleading negative build-reason UI label fixed and merged on `main` (commit `1bafd4728da5be4d7611ccae8014cbbee938dfcb`, verified 2026-09-25):** `BUILD_DISCOURAGED` now uses its own stable localization category (`Déconseillé` FR / `Discouraged` EN) instead of the positive `Build`; reason order retains the label in the existing two-label UI without changing scorer/resolver or introducing a new widget. Commit independently verified as exactly the four reviewed files `src/Localization.lua`, `src/UI.lua`, `tests/localization_spec.lua`, `tests/ui_spec.lua`, 40 additions / 2 deletions, one fast-forward commit after prior ROADMAP approval; remote `main` matched integration commit at verification. Tests added for French full `Core · Déconseillé` and English partial `Core · Discouraged`; preexisting positive `Core · Build` and conflict assertions preserved. Codex reports full Lua 5.2, canonical profile/mechanics, exact 14-file staging and diff checks PASS after integration; these were not independently executed here. Three historical review patches remain untracked locally per maintainer report and were not included. **DEV-only runtime recheck subsequently passed with screenshot and full `LogOutput(7).log` at 2:38:38**, with no Epic deploy, packaging or v0.2 release.
  - [x] **Merged `BUILD_DISCOURAGED` label fix deployed DEV-only via official transactional updater (Codex report reviewed 2026-09-25):** local `HEAD=origin/main=53e1f4e965fe9c6d2adfd138308f43aa9592095e` at deployment included code commit `1bafd472`; fast-forward only and all three untracked historic review patches preserved. Fresh persistent pre-deployment backup `D:\Dev\Games\HadesII-Dev\Ship\ReturnOfModding\backup\Local-HadesIIBoonAdvisor-pre-discouraged-20260925-023505` holds 14 files, immediately compared byte-identical to prior DEV target (`DIFF=0`). Staging 14, DEV 139606 compatibility and `tests/update_spec.ps1` under PS5.1 PASS per Codex. Official `tools/Update-BoonAdvisor.ps1` used established PS5.1 `-Confirm:$false` invocation; no manual file copy. Final DEV installed inventory 14, 13 non-settings hashes match staging, existing `config/settings.lua` byte-preserved with reported SHA-256 `087285B0176E195FAA1182396BC8D56390A0B9C1810C07E6E0D27BC97978F10F` (`DEBUG=true`, `UI_TEST_MODE=false`, `BUILD_PROFILE="auto"`), zero residual transactional directories or running Hades2 processes. Only actual pre/post target differences are `Localization.lua` and `UI.lua`, as intended; Epic reportedly untouched, no tag/package/release, working tree clean except three preserved untracked patches. These filesystem checks are **Codex-reported, not independently inspected here**; the merged four-file GitHub code commit was previously independently verified. The original fragile `ShouldProcess` invocation remains an independent installer reliability gate.
  - [x] **Post-deployment in-game `BUILD_DISCOURAGED` label smoke PASS on DEV-only (2026-09-25, screenshot plus complete `LogOutput(7).log`):** natural Morrigan Apollo boon screen after exactly two rerolls (three sequential evaluated offers, 2:38:33 / 2:38:35 / 2:38:38). Final screen at 2:38:38 visually shows `Frappe éclatante` Attack rank 1 (`Core`), `Technique éclatante` `ApolloSpecialBoon` rank 2 (`Core · Déconseillé`), and `Glyphe solaire` Cast rank 3 (`Conflit`). Full matching DEBUG shows `BuildId=sister_blades_morrigan_meta`, `ProfileMode=meta`, `RankingMode=full`; `ApolloSpecialBoon` score +6, covered+complete (+8 `FILL_EMPTY_PRIMARY_CORE` −2 `BUILD_DISCOURAGED`), rank 2, and exact `UI Reasons ... Labels=Core|Déconseillé`; no BoonAdvisor ERROR/WARN in complete supplied log. Two script-initialization announcements each report `Probe ready; hook count=1` (do not claim a single process-lifetime initialization). Only localization/UI changed in DEV per preceding Codex deployment; screenshot/log confirm the previous misleading `Core · Build` is fixed with no observed ranking regression. User confirmed game closed; no further gameplay is necessary for this fix. This checks the DEV smoke milestone only, not remaining unencountered RNG cases, primary save-backup integrity, independent Epic verification, packaging or v0.2 release.
  - [x] **Standalone `BUILD_DISCOURAGED` UI-label patch independently reviewed and approved for gated Git integration (2026-09-25):** uploaded `build-discouraged-11C11-review.patch` SHA-256 `6BCF3D9DC3E555CB671F54784F77F5BBBF43399AD5798718D9F6AC13DCC6688A`, exactly four expected files (`src/Localization.lua`, `src/UI.lua`, `tests/localization_spec.lua`, `tests/ui_spec.lua`), 40 additions and 2 deletions; preimage Git blob SHA prefixes match current remote baseline; since Codex's local `65dc3c7`, the remote `main` advanced only through ROADMAP.md. The change maps `BUILD_DISCOURAGED` to new FR `Déconseillé` / EN `Discouraged`, inserts it after Core/Build in UI reason order, and tests FR full-ranking plus EN partial-ranking labels alongside existing positive-build/conflict assertions; UI category rendering is shared between both paths. Generator independently rejects `core`/`preferred`/`discouraged` overlap for the same trait in a slot, so the new label is visible on the observed Morrigan Apollo case. This is a static patch review, **not** an independent execution of Codex's reported Lua 5.2/canonical/staging PASS. Cross-product FR-partial/EN-full and hypothetical future conflict+core+discouraged ordering remain potential extra defensive regression cases but are not blockers for the current three profiles. Next gate: safe fast-forward preserving all three local patch files, restore the reviewed four-file diff, rerun local tests, then commit/push exactly those four files. DEV retest and the 11C.11 milestone remain open; no new UI widget, scoring or Epic/live deployment authorized.
  - [x] **Morrigan later Ares-offer and second Hammer DEV screenshots checked against `LogOutput(5).log` (2026-09-25):** correct `BuildId=sister_blades_morrigan_meta`, `ProfileMode=meta` in all five observed offer evaluations; Ares `Source=AresUpgrade` first display and two rerolls at 1:38:42, 1:38:51 and 1:38:54. **Final photographed Ares offer (1:38:54) is Ares Mana `AresManaBoon`, NOT `AresSpecialBoon`**: score **−4**, `Alignment=NON_TARGET` / `SlotPolicy=reserved` / `SlotConflict=true`, displayed rank 2/2; `LowHealthLifestealBoon` score **+2** from `SURVIVAL_SUPPORT`, rank 1/2, condition `health_below_absolute:40` `CurrentlyActive=false` (support score does not mean its active effect is triggered); `OmegaDelayedDamageBoon` uncovered/incomplete and unranked; `RankingMode=partial`. Earlier Ares rerolls safely returned `RankingMode=partial` or `none` depending on number of fully evaluated offers. Second photographed hammer (1:39:57), `Source=WeaponUpgrade`: `DaggerChargeStageSkipTrait` (Cascade fulgurante), `DaggerSpecialJumpTrait` (Couteaux dansants), `DaggerTripleBuffTrait` (Gravures macabres) all `Score=0`, `Covered=false`, `Complete=false`; `RankingReady=false`, `RankingMode=none`, screenshot displays all unranked. `DaggerTripleBuffTrait` has documented native Blood Triad amplification but **is not in the approved executable Morrigan hammerPlan**; no scoring promotion authorized. Earlier owned `DaggerTripleRepeatWomboTrait` persists in this run, and `HephaestusSpecialBoon` remains equipped. Five total offers detected in supplied log, two hook-install announcements with `hook count=1`, 0 BoonAdvisor ERROR/WARN. **Morrigan Ares Special still has no live test** despite seeing Ares's Mana offer; all Blood Triad condition/payout modeling remains deferred. No new runtime changes, Epic deployment or release.
  - [x] **First Black Coat Melinoë DEV full-ranked Poseidon offer verified against screenshot and `LogOutput(6).log` (2026-09-25, 1:44:02):** `WeaponSuit` / `BaseSuitAspect`, `BuildId=black_coat_melinoe_intermediate`, `ProfileMode=intermediate`, 3/3 covered+complete, `RankingReady=true`, `RankingMode=full`; Rare `PoseidonWeaponBoon` Attack 13 (+8 empty primary/+4 core/+1 rarity), rank 1; `PoseidonCastBoon` Cast 8 (+8 generic empty primary/open slot; NOT a canonical core Cast), tied rank 2 with `PoseidonSprintBoon` 8 (+4 empty utility/+4 core), matching screenshot. An offer reroll at 1:50:28 upgraded Attack to Epic, score 14, same 1/2/2 order; the Epic Poseidon Attack is present in subsequent owned-trait snapshots. Initial `ForcePoseidonBoonKeepsake` owned, but **do not claim that this signal caused profile selection**: current generated registry has only one `WeaponSuit`/`BaseSuitAspect` candidate and resolver returns `only_candidate` before checking `autoSignals`. The signal is preserved, not causally validated; game set to auto per DEV configuration.
  - [x] **Black Coat live Hammer fallback and acquisition verified against screenshot and same-session log (1:51:15 onward):** three offered IDs `SuitSpecialBlockTrait` (Barrage punitif), `SuitAttackSpeedTrait` (Armature fulgurante), `SuitSpecialJumpTrait` (Missiles scintillants). Only the planned speed Hammer is covered+complete with `HAMMER_BUILD_PRIORITY=-2` (canonical ordinal priority 2; this is not a negative quality verdict); the other two are not covered, and `RankingMode=none` correctly displays `INCOMPLETE_ANALYSIS` with two unevaluated choices. `SuitAttackSpeedTrait` appears owned in subsequent snapshots, confirming acquisition. No new Hammers promoted and `Launcher Frame` still untested.
  - [x] **Black Coat live Ares core Special + secondary offers verified against screenshot and `LogOutput(6).log` (2026-09-25, 1:53–2:00):** final Ares screen at 2:00:29 matches `AresSpecialBoon` 12 (+8 empty Special/+4 core, covered+complete) rank 1/2, `AresSprintBoon` −4 (reserved Poseidon Sprint conflict) rank 2/2 and `BloodDropRevengeBoon` uncovered/incomplete, `RankingMode=partial`; screenshot displays 1/2, 2/2, unranked. Other logged offers: Aphrodite Heroic Attack replacing equipped Epic Poseidon Attack has `REPLACEMENT_UNRESOLVED`, score −4 from slot-policy conflict, uncovered/incomplete; her Sprint −4 complete, `ManaBurstBoon` unranked, `RankingMode=none`; Demeter Rare Special −3 vs reserved Ares Special, Epic Sprint −2 vs reserved Poseidon Sprint, `ReserveManaHitShieldBoon` unranked, partial ranking; Hephaestus Epic Cast +10 for open Cast (+8 empty/+2 rarity), other `AntiArmorBoon`/`EncounterStartDefenseBuffBoon` uncovered, ranking none. Log includes **seven** offer evaluations total (initial Poseidon, Poseidon reroll, Hammer, Aphrodite, Demeter, Hephaestus, Ares); no BoonAdvisor ERROR/WARN. Three hook/reinitialization announcements, each `hook count=1`, no duplicate offer event observed. Traits in later snapshots include Epic `ManaBurstBoon`, Rare `ReserveManaHitShieldBoon` and additional `SprintShieldBoon`/`SupportingFireBoon` acquired without scored offers in this log. These observations validate existing conservative fallback behavior, **not** missing support effects, autoSignal causality or the unresolved Launcher Frame condition.
  - [x] **Representative live DEV validation completed for all three v0.2 profiles on the pre-`BUILD_DISCOURAGED`-label-update runtime (2026-09-25):** user screenshots and corresponding full DEBUG logs independently checked for Sister Blades Melinoë (`AresWeaponBoon` Attack +13, full ranking), Sister Blades Morrigan (Hera Attack +13/full; Hephaestus Special +10 covered/complete with safe incomplete-offer fallback; two safely unranked Hammer screens; Apollo and Ares Mana partial-ranking/replacement screens), and Black Coat Melinoë (Poseidon Attack +13 then Epic +14/full; `SuitAttackSpeedTrait` correctly evaluated with two unknown Hammers and subsequently acquired; Ares Special +12, rank 1/2/partial). Correct weapon/aspect and per-offer `BuildId`/profile mode observed for each; fresh logs show `hook count=1` at each script initialization/reinitialization, not proof of a single hook over the entire process lifetime, and no BoonAdvisor ERROR/WARN. DEV primary Windows `Profile1`/cloud-save exposure was explicitly accepted by maintainer; the 14-file transactional DEV deploy and preserved `DEBUG=true` settings were previously verified by Codex report. **This checks off representative live coverage across all three profiles, not every named RNG-specific scenario or final release readiness.**
  - [x] **Morrigan victory run and Duo observation verified from fresh screenshot plus `LogOutput.log` (2026-09-25, log through 21:49:53):** victory screen confirms `DaggerTripleAspect` / Morrigan and the completed run. The photographed Duo corresponds to runtime ID `BloodRetentionBoon` (`Rarity=Duo`), seen at 21:40:40 alongside `AresSprintBoon` and `MissingHealthCritBoon`; it was `Covered=false`, `Complete=false`, so the UI correctly left it unranked and masked the ranking. The same run later retained `BloodRetentionBoon` while `DaggerTripleBuffTrait` and `DaggerAttackFinisherTrait` were owned (`HammerCount=2`). The log contains 16 detected offer evaluations, `BuildId=sister_blades_morrigan_meta` on the observed offers, and zero `[BoonAdvisor] ERROR` or `WARN` entries. This validates conservative handling and identity logging for the Duo; it does not promote Duo/Blood Triad mechanics into the v0.2 runtime.
  - [ ] **Optional RNG-dependent DEV observations (not blockers for the already-tested baseline or excluded features):** naturally encounter Melinoë Zeus Special and a representative Melinoë Hammer, Morrigan Ares Special or Black Coat Launcher Frame if convenient. Record unobserved cases as untested, never infer Blood Triad/Launcher predicates, and only make a case a v0.2 gate if its new behavior is specifically approved for runtime. Black Coat's Poseidon keepsake autoSignal causality cannot be isolated while this weapon/aspect has only one matching profile.
  - [ ] **DEV environment follow-up (separate from gameplay coverage):** the maintainer deliberately used primary Windows `Profile1` with cloud saves; the evidence does not establish an independently verified full save backup/integrity comparison. Codex reports DEV-only deployment and Epic untouched, but the Epic filesystem was not independently inspected. Record owner-accepted residual risk and/or verify backup/Epic integrity before further operations if needed; never describe the run as isolated.
  - [x] **Post-integration full offline regression matrix reported PASS by Codex (2026-09-25, tested `HEAD=origin/main=1eb5671dcf9a5f17a40e7014ee1af5ff6ffa77cc`):** Lua 5.2 complete, deterministic canonical generation, canonical profiles, strict mechanics equivalence, Build Registry and Black Coat importers, profile switcher, exact 14-file staging, patch compatibility and fixtures, install, update/rollback, uninstall, Thunderstore structure/inventory, release-packaging *fixture* with required `-ValidatedGameRoot` pointing to DEV, and `git diff --check` all reported PASS. First packaging-test invocation without the mandatory game-root parameter correctly refused to run, then passed on rerun with that parameter; it is **not evidence of a v0.2 release artifact or public packaging**. Codex reports no tracked edits and all three local untracked historic patches retained. Independent GitHub compare from tested commit `1eb5671` to subsequent `main` at audit time showed only `ROADMAP.md` changes, no runtime/test-code delta. These are Codex-reported test executions, not tests independently rerun here; **repeat final gate if executable code or packaging changes before actual release**. This closes the post-`BUILD_DISCOURAGED` offline regression milestone, **not** 11C.11's import decision, 11D public docs/version/release gates or future RNG-specific DEV observations.
  - [x] **Updater fixture diagnosis and safe DEV invocation independently compared with existing script contract (Codex report, 2026-09-25):** default unqualified PowerShell 5.1 call FAILED at first `ShouldProcess`; explicit `-Confirm:$false` PASS; `-WhatIf` PASS with no mutation; `AfterBackup` injected failure PASS with verified rollback; full transaction tests PASS. Existing `tools/Update-BoonAdvisor.ps1` retains `[CmdletBinding(SupportsShouldProcess=$true, ConfirmImpact='High')]` and all four guarded operations. No code patch, push or live deployment from this test. The public-facing documentation discrepancy remains a separate open 11D/update-doc task until reviewed.
  - [ ] **Document and disposition unqualified `ShouldProcess` invocation before public v0.2:** Codex's PowerShell 5.1 *temporary fixture* reproduced `NullReferenceException` at the first `$PSCmdlet.ShouldProcess` without `-Confirm`; the official updater succeeded with explicit `-Confirm:$false`; `-WhatIf` caused zero mutation, and injected `AfterBackup` rollback plus transactional suites passed. **Observed invocation dependence is established; the claimed precise internal cause (`$PSCmdlet` unavailable due to host) is not independently demonstrated from the report**, so do not assert root cause without an instrumented trace. No updater source patch is needed for the maintainer's established DEV command. However, public `docs/UPDATE.md` still shows the failing unqualified invocation; update it with a verified PowerShell 5.1 `-Command` example with escaped `-Confirm:$false`, explain the confirmation bypass and recommend `-WhatIf`/backup, or separately fix/test interactive invocation before release. Maintain `SupportsShouldProcess` and rollback behavior; a tested workaround is **not** proof that every PowerShell host/invocation works. No direct game installation or public release authorized by this fixture report.
...
- [ ] **Modernize public/project documentation for the finalized v0.2 scope.**
  ...
  - Modernize `docs/RUNTIME_TEST.md` for the current resolver/full/partial/localization/profile validation flow: its current introduction still says `NON EXÉCUTÉ` and prescribes an isolated Windows account, whereas later DEV tests knowingly used primary `Profile1` and cloud saves. Keep the isolated-account method as a safer alternative; do not falsely claim the completed tests were isolated.
  - Refresh or clearly mark historical sections in `docs/TECHNICAL_ANALYSIS.md` that describe obsolete defaults, profiles or staging counts: its original September 20 introduction says no scoring/mod was ever run. `README.md` must clearly separate the published v0.1.2 Starter support from future-v0.2 `main` where Starter was removed and Black Coat is active.
