...
- [ ] **11C.11 — Define and deliver an explicitly approved, evidence-gated v0.2 capability showcase for the three validated canonical profiles; 23-item metadata-only `keepsakePlan` completed.**
  ...
  - [ ] **Validate the five new recommendations in fresh DEV gameplay:** capture screenshots and complete DEBUG logs for correct profile/IDs, scores, coverage and full/partial/none fallbacks across natural Morrigan Hammer and Black Coat Attack/Hammer offers. If RNG does not provide an offer, record it as unobserved; do not infer Launcher Frame conditions or Blood Triad frequency. The installed build is ready, but `sky.launch_app` returned `Computer Use was not approved to use hades2`, so the game was not launched from this session. Resume when Windows Computer Use access to Hades is enabled, or use maintainer-captured gameplay evidence. The existing `config/settings.lua` uses the Windows `Profile1` with cloud saves; the maintainer previously accepted this DEV testing context.
