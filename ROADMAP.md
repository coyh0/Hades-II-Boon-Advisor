...
- [ ] **11C.11 — Define and deliver an explicitly approved, evidence-gated v0.2 capability showcase for the three validated canonical profiles; 23-item metadata-only `keepsakePlan` completed.**
  ...
  - [ ] **Decide the showcase UI scope before coding:** the maintainer previously requested a discreet, colored/icon-based build-status rectangle (detected profile; Attack/Special/Cast priorities and owned/missing state; existing phased Keepsake plan). In response to the newer v0.2 showcase request, explicitly ask whether a minimal read-only tile should move into v0.2 instead of the originally approved post-v0.2 UI phase. Do not silently revoke the earlier no-new-v0.2-UI constraint; Legendary/Duo paths require separate independently verified prerequisite and mapping evidence. Any new UI must be localized, unobtrusive, testable and informational-only.
