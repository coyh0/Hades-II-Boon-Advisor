...
- [ ] **11C.11 — Generate vetted canonical profiles and introduce informational `keepsakePlan`.**
  ...
  - [ ] **Fix misleading negative build-reason UI label without changing scoring:** `Localization.reasonKeys.BUILD_DISCOURAGED` currently emits the neutral/positive `build` label, making discouraged Apollo Special appear as `Core · Build` despite −2 build penalty. Review a concise localized FR/EN `discouraged`/negative-build label with deterministic UI regression tests; no new UI feature, no new scoring rule or deployment without patch review.
