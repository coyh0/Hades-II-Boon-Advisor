...
The repair is pushed on `codex/v02-mechanics-showcase`; draft PR [#1](https://github.com/coyh0/Hades-II-Boon-Advisor/pull/1) is open against `main` for shared review. It is not merged. Transactional deployment to DEV is complete with verified backup and exact inventory. Newly executable recommendations still require fresh DEV gameplay evidence before release. `sky.launch_app` was denied with `Computer Use was not approved to use hades2`; do not bypass this gate via shell or another route. Keep the compact UI after v0.2. Public updater documentation and other release-readiness work remain separate.
