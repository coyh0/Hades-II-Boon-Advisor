...
- [ ] **11C.11 — Define and deliver an explicitly approved, evidence-gated v0.2 capability showcase for the three validated canonical profiles; 23-item metadata-only `keepsakePlan` completed.**
  ...
  - [x] **Implement the five approved profile deltas and pass the local offline gate (2026-09-25):** the repaired branch preserves the five exact canonical/generated recommendations; restores the formerly corrupted roadmap, full scoring suite and canonical PowerShell test; and adds the five-row projection/policy plus fail-closed and scoring regressions. Canonical profiles, 23 Keepsake records, autoSignals, mechanics templates, scorer, resolver and UI were compared with clean `main`; only the five approved profile records changed. **15 suites PASS**: canonical/deterministic generation and full Lua 5.2; canonical mechanics/equivalence; Registry and Black Coat importers; v0.2 projection; profile switcher; 14-file staging; compatibility; install/update/rollback/uninstall; Thunderstore/package fixtures; and `git diff --check`. Windows PowerShell 5.1 child module paths were explicitly isolated after an initial test orchestration failure. Detailed evidence: `docs/V02_SHOWCASE_REPAIR.md`; machine results: `dist/repair-validation/results.json`. This closes local implementation/offline validation only; shared review/push, fresh live DEV validation and release remain open.
  - [x] **Complete local review and create the v0.2 candidate commit (2026-09-25):** consolidated diff against clean main is `dist/repair-validation/v02-showcase-consolidated.patch`; the repair is recorded in local commit `47f496b48b89e01c6519ad3d509215305c3cf031` on `codex/v02-mechanics-showcase`. Fifteen offline suites pass. The branch is one commit ahead of its local `origin` tracking ref. This does not represent a remote push, merge to `main`, DEV deployment or live validation. Preserve the three SHA-verified historical review patches. The attempted push was blocked by automatic review because remote ownership/trust was not established; do not retry by workaround.
  - [x] **Push the reviewed candidate branch for shared integration (2026-09-25):** pushed `codex/v02-mechanics-showcase` to the maintainer-confirmed repository and verified remote HEAD `9a8bef605dddbe88995575fd16d0eb188fa7544d`. Opened draft PR [#1](https://github.com/coyh0/Hades-II-Boon-Advisor/pull/1) against `main` for review. It is not merged; no DEV deployment or gameplay validation has occurred.
  - [ ] **Deploy the repaired v0.2 candidate to DEV and validate the new behaviors in gameplay:** use the official transactional updater and existing safe PowerShell 5.1 invocation, take/verify a fresh persistent pre-update backup, preserve DEV `config/settings.lua` and verify the exact 14-file inventory. Seek natural Morrigan Hammer and Black Coat alternative/Hammer offers; record screenshots plus full DEBUG logs, correct profile/IDs, scores, coverage, full/partial/none fallbacks and errors. If RNG does not provide a relevant offer, record it unobserved and rely only on the already-passing offline case; do not infer the Launcher Frame condition or Blood Triad frequency. This new-code deployment/gameplay check has not been authorized or performed yet.
