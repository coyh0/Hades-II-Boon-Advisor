...
- [ ] **11C.11 — Generate vetted canonical profiles and introduce informational `keepsakePlan`.**
  ...
  - [ ] **Black Coat Melinoë deep read-only audit, 35 records:** verify current canonical/native mappings, branch and Hammer conditions, and phase-aware informational Keepsakes, then reconcile all three per-profile audit reports.
  - [ ] **Reconcile Registry v2 tracking metadata with the maintainer's completed documentary content approval:** remove the obsolete content-approval-pending state only via an authorized, audited Sheet update. Preserve per-row `unresolved` recommendations, unverified native IDs/mechanics, ambiguous conditions and `documentation_only`; never bulk-mark mechanical verification or runtime readiness.
