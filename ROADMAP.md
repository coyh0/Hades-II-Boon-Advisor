...
- [ ] **11C.11 — Generate vetted canonical profiles and introduce informational `keepsakePlan`.**
  ...
  - [ ] **Local implementation reported by Codex (awaiting independent patch review):** eight tracked files modified only (three canonical JSON profiles, three corresponding generated Lua modules, generator `tools/Generate-BoonAdvisorProfiles.ps1`, and `tests/canonical_profiles_spec.ps1`); optional four-phase `keepsakePlan` metadata, non-executable text and retained `recommendationId`; no change claimed to runtime scoring, resolver, slots, Hammers or existing Black Coat Poseidon autoSignal. Codex reports canonical PowerShell 5.1, deterministic generation, JSON/Lua parity, Lua 5.2, 14-file staging and `git diff --check` PASS. These are Codex-reported tests, not independently rerun; **do not check the implementation milestone complete or authorize commit/deploy until the complete eight-file diff and evidence have been reviewed**. Codex local HEAD was `70691e2`, behind current remote ROADMAP-only commits; preserve untracked `ares-11C8-review.patch` before any later safe sync.
  - [ ] **Independent patch review: corrections required before approval:** the uploaded UTF-8/no-BOM patch SHA-256 `986077C6E74D7D1EEDAC6FA2EEF7D92009CF460C38703D6C716E8BD2C8CE8680` has exactly the expected eight paths (321 additions, no deletions) and matching GitHub preimage blob SHA prefixes. All 23 new Keepsake entries match their generated Lua representations; no old runtime-driving JSON fields were changed. Direct comparison with the current 378-row approved Registry v2 found **four incorrect `documentaryPriority` copies**: Melinoë R3 Experimental Hammer 1→2, Morrigan R2 Harmonic Photon 1→2, Morrigan R3 Knuckle Bones 1→2, Morrigan R3 Luckier Tooth 1→2 (values shown are patch→Sheet). The new missing-phase test fixture introduces malformed JSON, so it can pass without exercising the intended phase validator. Generator checks nonempty and duplicate phase-local trait IDs, but does not attest native ID trust or Registry recommendationId membership. Correct the four copies, make the phase-negative test valid JSON with targeted failure evidence, strengthen regression coverage for all three profiles, and resubmit the patch; do not commit/deploy yet.
