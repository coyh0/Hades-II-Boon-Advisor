...
- [ ] **11C.11 — Generate vetted canonical profiles and introduce informational `keepsakePlan`.**
  ...
  - [ ] **Unblock verified transactional DEV deployment:** reproduce first-ShouldProcess behavior in an isolated fixture under the exact failing host/invocation, compare direct Windows PowerShell 5.1 script invocation, confirm whether a minimal tested invocation-only change suffices or a narrowly reviewed updater fix is needed; rerun fixture update/rollback tests. With unchanged target and preserved backup, retry only the authorized DEV target via the existing updater, `-Confirm:$false` where appropriate, then verify 13 staged-file hashes plus byte-identical preserved DEV settings; collect new game-session log/screenshots for each of the three profiles. No Epic or release actions.
