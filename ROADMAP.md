...
Community Curator has completed the private Sheet migration and reported successful formula/contrast QA: six Intermediate guides and six Meta guides, 378 atomic DATA rows in total (260 new). The pilot/prototype tabs were promoted and cleaned up; the Build Registry remains untouched. **11C.9 human-guide V2 visuals are maintainer-approved; pilot provenance cleanup, final DATA recommendation approval and explicit v0.2 profile-scope decision remain open. DATA coverage is not runtime readiness.** Registry v2, `keepsakePlan`, and broader profile promotion follow that gate. Historical 15-file, Starter and pre-audit statements below describe past milestones, not the current runtime. The 2-of-3-evaluable Hammer case remains an optional live observation; Hammer rerolls are not a game mechanic.
...
- [ ] **11C.9 — Accept completed Community Curator migration, resolve pilot provenance gaps, and freeze the intended v0.2 profile set.**
  ...
  - [ ] Resolve or explicitly mark not-applicable the historical pilot provenance blanks: Intermediate has 19 missing source dates and 12 missing source URLs; Meta has 15 dates and 8 URLs missing. Do not invent dates/URLs for owner preferences or unverifiable archive references.
  - [ ] Approve the final DATA recommendations and select the actual `v0.2` runtime profiles. Curator documented twelve builds, but source rows do not become validated runtime profiles automatically.
  ...
  - Optional UX follow-up: add a compact clickable index/jump links for the six vertically stacked guide cards per human tab, without changing the approved V2 card design.
