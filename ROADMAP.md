...
- [ ] **11C.11 — Generate vetted canonical profiles and introduce informational `keepsakePlan`.**
  ...
  - [ ] **Remaining targeted DEV evidence and environment closure (do not block the already-validated three-profile baseline):** Melinoë Zeus Special and a representative Melinoë Hammer, Morrigan Ares Special, and Black Coat conditional Launcher Frame were **not observed** in the supplied matching live captures/logs; test only when naturally offered, report each as untested meanwhile, and never infer Blood Triad/Launcher predicates. Black Coat `ForcePoseidonBoonKeepsake` was equipped, but its autoSignal causality is unproved because the resolver has a singleton compatible Black Coat candidate. Recheck a current backup/integrity of primary saves and verify Epic/live install remained unchanged rather than assuming this from gameplay logs; no public release. The merged `BUILD_DISCOURAGED` localization fix has now passed its own in-game DEV screenshot/log smoke check; no repeat needed.
  - [ ] **Final post-integration regression/release gate still open:** rerun deterministic canonical/Lua generation, Build Registry importer, scoring/resolver, full Lua 5.2/UI, exact 14-file staging, compatibility, install/update/uninstall and packaging regressions on the latest reviewed `main` before v0.2 release. Codex-reported full Lua 5.2, canonical profiles/mechanics, staging and diff checks passed after commit `1bafd47`; the **entire** install/update/uninstall + packaging matrix has not been demonstrated after that integration. Independently resolve or document the first-`ShouldProcess` CLI reliability defect; no release/tag/Epic deployment authorized.
