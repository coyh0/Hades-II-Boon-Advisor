...
**11C.11 — Define and verify a meaningful v0.2 capability-showcase scope across the three existing profiles; metadata-only `keepsakePlan` already delivered**
...
Community Curator completed the private Sheet migration and the maintainer approved all **378** documentary recommendations across 12 guides (118 in the three approved v0.2 profiles; 260 deferred); 11C.9/11C.10 are complete. All Registry v2 rows remain `documentation_only`; documentary approval does not independently validate every native ID/mechanic or make a row runtime-importable. The 118-row per-profile read-only inventory and native triage are complete. All **23** four-phase `keepsakePlan` metadata records (8 Melinoë / 9 Morrigan / 6 Black Coat) were merged into canonical and generated profiles, structurally regression-tested per Codex, and deployed as part of DEV modules; they are **informational, non-executable and have no v0.2 UI**. The maintainer rejected a zero-addition freeze on 2026-09-25: v0.2 should visibly demonstrate more of the mod's capabilities before release. An evidence-gated curated V2→V1 runtime delta and the potential inclusion of the previously proposed compact informational build-status tile must be scoped and explicitly approved before implementation; do not import all 118 rows wholesale or assume permission for new UI/scoring. The 34 unknown historical source dates are optional and non-blocking. Historical 15-file and Starter statements below describe earlier milestones, not the current runtime. Hammer rerolls are not a game mechanic.
...
- [ ] **11C.11 — Define and deliver an explicitly approved, evidence-gated v0.2 capability showcase for the three validated canonical profiles; 23-item metadata-only `keepsakePlan` completed.**
  ...
  - [ ] **Decide remaining v0.2 source→runtime deltas before any import:** Melinoë's Ares/Wounds discrepancy is already resolved by 11C.8 and live validation; do not reintroduce a prerequisite for base `AresWeaponBoon`. Existing Morrigan preferred Special and Black Coat Poseidon/Ares/Speed-Hammer baseline have representative DEV evidence. For any *new* Morrigan Hammer/Special classification or Black Coat Hestia/Zeus/Reaper Frame/Launcher conditional promotion, require explicit maintainer approval plus native ID/mechanics evidence; otherwise defer as `documentation_only`, not a blocker for the validated baseline.
  ...
  - [x] **Targeted read-only native-evidence triage received (Codex, report reviewed 2026-09-25):** local `origin/main` synchronized to `2645cfd` with both historical review patches preserved and no tracked edits. Native file locations/line numbers reported for existing Morrigan Ares/Hephaestus Specials, selected unplanned dagger Hammers and Blood Triad text; Black Coat Hestia/Zeus alternatives and Cast, Reaper Frame and the existing conditional Launcher Frame. This is Codex-reported native evidence, not independently re-read DEV source or live validation. Codex proposes **no scoring/canonical changes at this gate**: keep existing Morrigan Specials and Black Coat baseline; defer all new Hammers, alternative promotions, Blood Triad predicates, and Launcher Frame condition execution pending targeted mechanic/owner decision. Regression requirements from that report were subsequently compared with the existing tests and the offline suites rerun successfully, per Codex's 2026-09-25 report; live DEV verification remains outstanding. Melinoë base Ares 11C.8 and all three metadata-only `keepsakePlan`s remain unchanged; 260 future records deferred. Main 11C.11 projection and DEV tests remain open.
  ...
  - [x] **Maintainer v0.2 showcase UI scope decided (2026-09-25):** concentrate v0.2 on stronger *verified* recommendations, mechanics and differentiated behavior across the three active profiles. **No new compact build-status tile, phased Keepsake HUD, Legendary/Duo prerequisite panel or other new build-guidance UI in v0.2**; retain the existing offer ranking and localized diagnostic labels. The previously proposed discreet colored/icon tile remains planned for the post-v0.2 UI-polish/build-guidance phase. `keepsakePlan` remains metadata-only in v0.2. This is a scope decision, **not approval to promote any specific Registry v2 row or change scoring**; native IDs/mechanics, precise proposed projection, focused tests, targeted DEV validation and separate patch review are still required.
  ...
  - [ ] **Propose, review and approve a *curated nonzero* Registry v2→V1 runtime-item projection for v0.2:** the maintainer explicitly rejected a zero-addition freeze and wants to demonstrate capability before release. Identify a small, useful, high-confidence subset of the 118 documentary rows for three distinct profiles, rather than bulk import. Compare each candidate with the current executable baseline and independently verify native IDs/mechanics, conflict/conditional behavior, regression coverage and targeted DEV tests before enabling it. Each promotion requires independently verified IDs/mechanics, policy, regression tests and targeted DEV evidence. The separate four-phase `keepsakePlan` mapping is already implemented as 23 non-executable metadata entries; it does not belong to the V1 importer. Keep all other recommendations documentary-only and preserve existing validated scoring.
  - [ ] **Prepare a three-profile recommendation/mechanics showcase proposal before coding:** compare the 118 documentary v0.2 records to the current canonical/runtime behavior; identify only meaningful, independently verifiable additions (e.g. explicitly approved Morrigan Hammer choices/conditional Blood Triad behavior and Black Coat alternative Boons/Hammers if mechanics truly support them), distinguish already implemented recommendations from new executable deltas, give exact internal IDs and native references plus conservative fallback for unresolved conditions, and specify per-profile scoring/resolver/UI-label regressions and realistic DEV test scenarios. Preserve the already-validated Melinoë Ares base Attack and all existing reliable rankings. Require maintainer review of the proposed *small selection*, not 118 individual documentary rows. The nine other profiles and 260 future rows remain deferred.
