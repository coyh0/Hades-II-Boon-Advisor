...
- [ ] **11C.11 — Generate vetted canonical profiles and introduce informational `keepsakePlan`.**
  ...
  - [x] **Local metadata-only implementation submitted and independently patch-reviewed (integration pending):** eight tracked files modified only (three canonical JSON profiles, three corresponding generated Lua modules, generator `tools/Generate-BoonAdvisorProfiles.ps1`, and `tests/canonical_profiles_spec.ps1`); optional four-phase `keepsakePlan` metadata, non-executable text and retained `recommendationId`; no change claimed to runtime scoring, resolver, slots, Hammers or existing Black Coat Poseidon autoSignal. Codex reports canonical PowerShell 5.1, deterministic generation, JSON/Lua parity, Lua 5.2, 14-file staging and `git diff --check` PASS. These are Codex-reported tests, not independently rerun; the complete revised eight-file diff has now been statically reviewed; integrate only after safe Git synchronization and a fresh full local test run, with DEV/Epic deployment still unauthorized. Codex local HEAD was `70691e2`, behind current remote ROADMAP-only commits; preserve untracked `ares-11C8-review.patch` before any later safe sync.
