...
- [ ] **11C.11 — Generate vetted canonical profiles and introduce informational `keepsakePlan`.**
  ...
  - [ ] **Morrigan Apollo live-offer screenshot received (2026-09-25; corresponding fresh DEBUG log still required):** shows `CLASSEMENT PARTIEL` for 2/3 evaluable items: Heroic Apollo Attack `Frappe éclatante` replacing a displayed existing Hera Attack ranks **2/2**; Apollo Special `Technique éclatante` ranks **1/2** with UI reasons `Core · Build`; Apollo `Double Dose` is unevaluated. The full/partial fall-back display appears correct, but **the `Core · Build` annotation for Apollo Special is inconsistent with the current Morrigan canonical profile, where `ApolloSpecialBoon` is `discouraged` (not `core`/`preferred`)**, and could indicate an unexpected profile/resolver decision, stale UI, different offer ID or a runtime scoring defect. Do not mark this offer PASS until the same-session log confirms `BuildId`, `ItemName`, `TraitToReplace`, ranking reasons and scoring for all three offers. Preserve screenshot/time evidence; no scoring changes on visual evidence alone.
