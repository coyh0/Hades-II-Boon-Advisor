...
**11C — Ares correction, accepted V2 guide visuals and documentary Registry v2 migration**
...
The first live Melinoë Ares offer displayed Vicious Strike as unevaluated. Native DEV evidence (game executable version `139606`) confirms the basic Ares Attack independently applies Rend and does not require Grievous Blow. Codex reports that the targeted canonical/profile/test correction has been made locally with offline regression suites passing, but the five-file patch remains uncommitted, has not been independently reviewed from the local diff, and has not been deployed or live-validated. Origination-category combination behavior remains unresolved; do not infer additional synergy rules.

Community Curator completed the private Sheet migration and the maintainer approved the V2 visuals: six Intermediate guides and six Meta guides, 378 atomic DATA rows in total (260 new). **11C.10 is complete:** a separate 49-column `Build Registry v2` now holds all 378 documentary recommendations (118 v0.2-scope rows; 260 deferred), while the legacy V1 Registry and its archive remain intact. **11C.9 remains open only for final recommendation/content acceptance and optional verification of 34 historical source dates currently marked unknown; all 20 missing public URLs are explicitly not applicable to owner preferences or Curator inferences.** The three-profile v0.2 scope is maintainer-approved. All Registry v2 recommendations remain `documentation_only`; native ID verification, an approved V2→V1 projection, and `keepsakePlan` belong to 11C.11. DATA coverage is not runtime readiness. Historical 15-file, Starter and pre-audit statements below describe past milestones, not the current runtime. The 2-of-3-evaluable Hammer case remains an optional live observation; Hammer rerolls are not a game mechanic.
...
- [ ] **11C.8 — Fix Ares base-branch scoring and rerun realistic live DEV validation.**
  ...
  - [x] **Local corrective patch reported by Codex:** `AresWeaponBoon` moved from conditional `WOUNDS_ACCESS` to `alternative` with ordinal priority 2 in canonical Intermediate JSON and regenerated Lua; `ScoringEngine.lua`, Origination rules, other profiles and five Hammer choices unchanged. Five local files changed in total (including three test files); no commit/push/deployment.
  - [x] **Offline validation reported by Codex:** Lua 5.2 suites, targeted replacement/Origination/regression cases, canonical deterministic generation and equivalence, import, resolver/profile switcher, staging/package inventory, install/update/uninstall, patch compatibility and `git diff --check` passed. Staging contains 14 files; generated/staged Intermediate Lua SHA-256 matched. The local diff and raw test logs have not been independently inspected here.
  ...
  - [ ] Reconcile Codex's local `3201282` base with newer GitHub documentation commits, verify the patch against the integrated tree and rerun tests before any authorized commit/push.
  - [ ] With explicit maintainer authorization, deploy to DEV only, check live Ares offer/profile/ranking/UI/logs, and rerun relevant post-deploy regressions. Do not touch Epic/live automatically.
- [ ] **11C.9 — Accept completed Community Curator migration, resolve pilot provenance gaps, and freeze the intended v0.2 profile set.**
  ...
  - [ ] **Optional source-date verification before final provenance acceptance:** research the 34 unknown publication dates only against reliable page/archive metadata; keep unresolved values `unknown` rather than filling them from the current page date. This is not a blocker for documentary Registry v2 records.
