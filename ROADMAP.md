...
- [ ] **11C.11 — Generate vetted canonical profiles and introduce informational `keepsakePlan`.**
  ...
  - [ ] **Live three-profile DEV validation after successful deploy:** use the maintainer-acknowledged primary-profile DEV test context (or an isolated account if they later choose), launch only the DEV executable, confirm exactly one hook and `Probe ready; hook count=1`, capture fresh timestamps/screenshots and DEBUG log excerpts for Melinoë Ares/Zeus Special and representative Hammers, Morrigan Special Ares/Hephaestus as naturally offered, and Black Coat Poseidon start/Ares Special/Launcher Frame when encountered. Report unencountered RNG cases as untested; do not claim the unimplemented Blood Triad/Launcher conditions verified or infer selection via keepsake without an explicit resolver reason. Recheck primary saves and absence of Epic changes; no public release.
